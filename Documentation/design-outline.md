...
> **Status:** Draft **v0.21** — architecture outline; Milestone A of `tasks-server.md` is built.
...
- **The join code is returned to the organiser only.** *(Implemented in SRV-20.)* `RideDetail.JoinCode` is `null` for every member who is not the organiser. The code is the ride's entire access control, so a member's copy carrying it would let any member re-share the group the organiser curated — which quietly undoes the guarantee the rest of this section is built on.
...
- **A home private area — a point and a radius set on the device (§18.6), inside which nothing is recorded and nothing is broadcast.** Unlike the display rule above this one is *collection*: the fix never becomes a stored row and never reaches the hub, so there is nothing to hide later. Co-riders see the rider as present in the ride with no position on the map. Three properties are load-bearing and are asserted rather than assumed:
  - **The area itself never leaves the device.** Its centre is a statement of where somebody lives, so the only copy that cannot leak is the one the server is never told about — including the fact that one is set at all. The cost is stated in the UI rather than hidden: it does not follow the rider to another phone, and a reinstall forgets it.
  ...
  - **The gate is closed until the setting has been read.** "No area is set" and "we have not looked yet" are different states, and only the first is safe to publish from.
