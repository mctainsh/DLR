...
**The disclosure comes before all of it** *(v0.34)*. `BlazorDLR.Shared/State/LocationDisclosure.cs` holds Play's prominent disclosure — the copy, the key it is remembered under, and the ask — and every route to a fix goes through it before `EnsurePermissionsAsync` is reached: the broadcaster, and *My routes → Use my location*. It names **both** uses, because a fix is published *and* recorded: the receiver offers every fix to the recorder (§15.1) upstream of any publish gate, and a disclosure that mentions only sharing is describing half of what the app does with the data. That half-description is what 8.0.0.28 was rejected for (§10.2, `Documentation/store-release.md`).
