...
| `INotificationService` | FCM on Android, APNs on iOS | No-op (§18.2) | Ride thread; the ongoing "still sharing your location" notification |
...
- [x] `ConsentPrompt.razor` — the two-choice location prompt (§5.6). Shown once per adventure per device, for a rider who does not yet share; the "asked already" fact lives in `IDeviceSettings` under `dlr.consent.asked.{rideId}`, because ride state used to carry half of it.
- [x] `GroupRideLive.razor` — hosts `RideMap` with member positions + markers via a single Skia layer, `MemberList`, share-toggle, organiser controls.
- [x] `MemberList.razor` — three states per member: *sharing*, *not sharing*, *no signal* (§5.6). Distinct, never collapsed.
- [x] `IRideHubClient` extended with every server-side `IRideClient` event: positions, member join/leave, marker CRUD, comment CRUD, reactions, poll updates, permissions, member sharing.
...
- [ ] The ongoing "still sharing your location" notification on the phone. **Not closeable from this environment** — needs the platform-native notification service (FCM / APNs), a real device, provider registrations, and the recording session that owns the state. It matters more since the adventure lost its end (§5.6): the rider's own switch is now the only thing that stops sharing, so the app owes them a standing reminder that it is on.
...
- [x] `RideThread.razor` — pinned first, cursor-paginated (`CommentPage.NextCursor`), text + photo composer, reactions with the coalesced hub message updating counts in place, edit within window (server enforces), pin/delete for organiser/leader, report for others. The "quiet when Live" rule was removed in v0.26 and the adventure state it keyed on in v0.32; every post notifies.
- [x] `PermissionsChanged` wires into `GroupRideLive` — the compose surface for markers/thread/photos honours `RidePermissions` on the ride detail and re-renders on the message (§5.3, §5.8).
...
- [x] ~~Ride end flow~~ — deleted with the lifecycle. Deleting the adventure is the only ending, and it is the organiser's own confirmed action (§5.1, §5.6).
...
**Exit criterion**: four people, one real ride. One joins by code, one is admitted from a request, one joins without sharing and stays invisible on the map while seeing everyone. One turns sharing off mid-ride and their pin goes from all three other phones at once. Trim your own house off a real recorded ride on the web editor.

**Phase 2 status (this pass):** the online / realtime / social side is done. Auth, group ride lifecycle, live map with pins + markers + Skia overlay, thread with reactions, moderation, permissions, web track editor. What is deferred to Phase 3 is the same platform-native work Phase 1 deferred — the foreground location service on Android, `CLLocationManager` on iOS, the ongoing "still sharing" notification. `PublishPositionAsync` is wired end-to-end at every seam except the recorder that would call it.
