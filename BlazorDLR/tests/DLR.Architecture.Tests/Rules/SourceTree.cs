...
{
	/// <summary>The directory containing <c>BlazorDLR.slnx</c>.</summary>
	...
	{
		foreach ((string prefix, string directoryName) in TopLevels)
		{
			...
			{
				// Rules assert against paths starting with "src/" or "tests/". Remap each
				// physical folder onto that view: the file's path relative to the folder is
				// appended to the folder's logical prefix.
				string physical = Path.GetRelativePath(directory, path).Replace('\\', '/');
				string relative = prefix + physical;
	...
	private static string FindRoot()
	{
		...
		while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "BlazorDLR.slnx")))
		{
		...
		return directory?.FullName
			?? throw new InvalidOperationException(
				$"Walked up from '{AppContext.BaseDirectory}' without finding BlazorDLR.slnx, so the source rules " +
