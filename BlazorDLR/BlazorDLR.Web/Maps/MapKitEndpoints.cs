...
[ApiController]
public sealed class MapKitController : ControllerBase
{
	[HttpGet("/api/v1/maps/token", Name = MapKitEndpoints.RouteName)]
	// Authenticated, per §4.5. A public token endpoint is a free map quota for the
	// internet, billed to us.
	[Authorize]
	[EndpointSummary("A short-lived MapKit JS token. The signing key never leaves the server.")]
	public IActionResult Get(
		[FromServices] IOptions<MapKitOptions> options,
		[FromServices] MapKitSigningKey keys,
		...
		[FromServices] ILoggerFactory loggers)
	{
		...
		if (!throttle.TryAcquire(
			$"maps-token:{HttpContext.Connection.RemoteIpAddress}",
		...
		if (!settings.IsConfigured)
		{
			...
			return new ObjectResult(new ProblemDetails
			{
				...
				Title = MapKitEndpoints.NotConfiguredTitle,
		...
		try
		{
			...
			return Ok(new MapToken(token, expires));
		}
		catch (Exception exception) when (exception is CryptographicException or ArgumentException)
		{
			...
			loggers
				.CreateLogger(typeof(MapKitController))
