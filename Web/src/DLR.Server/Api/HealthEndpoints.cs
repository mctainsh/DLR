...
public static class HealthEndpoints
{
	...
	{
		endpoints
			.MapGet("/healthz", GetAsync)

			// Anonymous, because the thing watching it is a free uptime pinger with no credential
			// to present. That is also why the body says as little as it does.
			.AllowAnonymous()
			.WithName(RouteName)
			.WithSummary("Whether this server can serve: database, schema and disk.");
	...
	private static async Task<IResult> GetAsync(
		...
		IOptions<BlobStoreOptions> blobs,
