...
public static class TrackImportEndpoints
{
	...
	/// <summary>Maps <c>POST /api/v1/tracks/import</c>.</summary>
	public static IEndpointRouteBuilder MapTrackImport(this IEndpointRouteBuilder endpoints)
	{
		endpoints
			.MapPost("/api/v1/tracks/import", ImportAsync)
			.RequireAuthorization()
			.DisableAntiforgery()
			.WithName(ImportRouteName)
			.WithSummary("Reads a GPX file. ?dryRun=true previews without persisting.");
		...
		bool dryRun = false)
	{
		...
		if (dryRun)
		{
			...
			return Results.Ok(new TrackImportResult(
				DryRun: true,
				results,
	...
	private static IResult TooLarge(TrackImportOptions caps) => Problem(
		StatusCodes.Status413PayloadTooLarge,
		"File too large",
