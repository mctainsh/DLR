...
public static class AccountEndpoints
{
	...
	/// <summary>Maps both endpoints.</summary>
	public static IEndpointRouteBuilder MapAccount(this IEndpointRouteBuilder endpoints)
	{
		endpoints
			.MapGet("/api/v1/me/export", ExportAsync)
			.RequireAuthorization()
			.WithName(ExportRouteName)
			.WithSummary("Everything this server holds about the caller, as a ZIP archive.");

		endpoints
			.MapDelete("/api/v1/me", DeleteAsync)
			.RequireAuthorization()
			.WithName(DeleteRouteName)
			.WithSummary("Deletes the account, its content and its blobs. Irreversible.");
		...
		CancellationToken cancellationToken)
	{
		...
		ILogger logger = loggers.CreateLogger(typeof(AccountEndpoints));
