	...
	ILogger<RefreshTokenService> logger)
{
	...
	{
		(_, string raw) = Issue(userId, deviceId, Guid.NewGuid());
	...
	{
		(RefreshToken successor, string raw) = Issue(token.UserId, token.DeviceId, token.FamilyId);
	...
	/// </summary>
	private (RefreshToken Entity, string Raw) Issue(Guid userId, Guid deviceId, Guid familyId)
	{
		...
		RefreshToken token = new()
		{
			...
			TokenHash = Hash(raw),
			IssuedUtc = now,
			ExpiresUtc = now.AddYears(options.Value.RefreshTokenYears),
		};
