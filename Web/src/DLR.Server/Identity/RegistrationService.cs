	...
	TimeProvider clock)
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		{
			return RegistrationOutcome.Refused(Results.ValidationProblem(
		...
		if (!result.Succeeded)
		{
			return RegistrationOutcome.Refused(Results.ValidationProblem(AsProblems(result)));
		}
...
public readonly record struct RegistrationOutcome(AppUser? User, IResult? Problem)
{
	...
	public static RegistrationOutcome Refused(IResult problem) => new(null, problem);
}
