...
public static class MarkerEndpoints
{
	...
	/// <summary>Maps the marker endpoints.</summary>
	public static IEndpointRouteBuilder MapMarkers(this IEndpointRouteBuilder endpoints)
	{
		endpoints
			.MapPost("/api/v1/markers", CreateAsync)
			.RequireAuthorization(AuthorizationPolicies.NotRestricted)
			.WithName(CreateRouteName)
			.WithSummary("Places a marker on a track or a ride.");

		endpoints
			.MapGet("/api/v1/group-rides/{id:guid}/markers", RideMarkersAsync)
			.RequireAuthorization()
			.WithName(ListRouteName)
			.WithSummary("Every marker on a ride.");

		endpoints
			.MapPut("/api/v1/markers/{id:guid}", UpdateAsync)
			.RequireAuthorization()
			.WithName(UpdateRouteName)
			.WithSummary("Edits a marker.");

		endpoints
			.MapDelete("/api/v1/markers/{id:guid}", DeleteAsync)
			.RequireAuthorization()
			.WithName(DeleteRouteName)
			.WithSummary("Removes a marker.");
		...
		endpoints
			.MapPatch("/api/v1/markers/{id:guid}/photo", AttachPhotoAsync)
			.RequireAuthorization()
			.WithName(AttachPhotoRouteName)
			.WithSummary("Attaches an uploaded photo to a marker, or detaches the one that is there.");
		...
		TimeProvider clock)
	{
		...
		IResult? refused = request.GroupRideId is { } rideId
	...
	private static async Task<IResult> RideMarkersAsync(
		Guid id,
		ClaimsPrincipal caller,
		DlrDbContext database)
	{
	...
	private static async Task<IResult?> CheckRideAsync(
		DlrDbContext database,
		Guid rideId,
		Guid userId,
	...
	private static async Task<IResult?> CheckTrackAsync(
		DlrDbContext database,
		Guid trackId,
		Guid userId,
		MarkerOptions limits)
